---
title: "Build Your Own Text Prediction Application"
...
When we type sentences in our computers or handhelds, it is often helpful if the application could figure out what we are tying to type and suggest next few words. Google Search and Swiftkey Keyboards are very good example of this usecase. What if we could create an application which does exactly that and is personalized to our own typing pattern? It would be exciting right? In computer science, Natural Language Processing is where Language Models are engineered. This kind of application can be created with NLP and powerful languages like R. In this blog, we will create NextIT, a Word/Sentence Prediction App using NLP in R.


## Synopsis

In this blog we will try to understand and build predictive text models that will help users in typing sentences faster in keypads. A typical example is the SwiftKey Keyboard in mobile devices. We will start with the basics, analyze a large corpus of text documents to discover the structure in the data and how words are put together. We will cover from cleaning and analyzing text data, to building and sampling from a predictive text model. 

Requirements

1) R
2) R Studio
3) tm, RWeka libraries
